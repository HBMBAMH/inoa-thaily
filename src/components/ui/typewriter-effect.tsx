...
export const TypewriterEffect = ({
  ...
  useEffect(() => {
    ...
    const playAnimation = async () => {
      ...
      timeout = setTimeout(playAnimation, 3000)
    }
            ...
            <div key={`word-${idx}`} className="inline-block">
              {word.text.map((char, index) => (
                <motion.span
                  ...
                  key={`char-${index}`}
                  className={cn(
                    `cormorant-garamond hidden opacity-0 dark:text-primary`,
                    word.className
                  )}
                >
                  {char}
  ...
  return (
    <div
      className={cn(
        "text-center text-5xl font-bold sm:text-xl md:text-3xl lg:text-5xl",
        className
      )}
    >
      {renderWords()}
